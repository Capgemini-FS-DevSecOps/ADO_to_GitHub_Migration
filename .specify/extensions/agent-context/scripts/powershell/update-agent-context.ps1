...
if ($null -eq $Options) {
    ...
    if ($pythonCmd) {
        try {
            $pyOneLine = "import json,sys,yaml; d=yaml.safe_load(open(sys.argv[1],encoding='utf-8')); print(json.dumps(d if isinstance(d,dict) else {}))"
            $jsonOut = & $pythonCmd -c $pyOneLine $ExtConfig
