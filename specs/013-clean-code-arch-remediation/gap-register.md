...
one of the 97 entries carries at least one path:line citation or a reproduction command
...
- reopened_and_closed: 2026-09-13, same day, by Claude (opus subagent, R10b), commit `9719609`. A second read-only review found the first fix incomplete: the probe normalised the whole candidate, but httpx drops everything from the first `#` before it builds the request path, so a `..#` read as one ordinary segment to the check while the wire saw a bare `..`. Measured on the installed httpx 0.28.1 — `/v1/ado/..#` has probe `/v1/ado/..#`, which passes the prefix test, and `Client(base_url=...)._merge_url('/v1/ado/..#')` sends `/v1`. `..#junk` and `x/..#` behave the same. `join_api_path` now refuses any candidate containing `#`, checked on both the raw candidate and the percent-decoded path, so `..%23` is refused too even though that one is harmless on the wire; a `#` is never legitimate in an API path here, and with it gone the probe and the transport path agree. Verified by sweeping 23 adversarial endpoints — `%2523`, `%25%32%33`, overlong UTF-8 `%c0%af`, `....//`, `..;/`, NUL, newline, a query-only endpoint, `?#/../..` — across both prefixes against the real `httpx.Client._merge_url`: no accepted input leaves its prefix, and legitimately percent-encoded ADO names still pass through byte-identical. Regression: `tests/unit/test_tool_bindings.py::test_join_api_path_rejects_fragment_assisted_traversal` (4 cases) and `::test_join_api_path_agrees_with_what_httpx_would_send` (9 endpoints x 2 prefixes, asserted against real httpx rather than against a restatement of the implementation). Revert proof at `run-r10b2-revert-tools.txt`, `16 failed, 37 passed`.
...
- audit: refusals were originally visible only as a tool result — to the model and to nobody else. `_refuse_path` (`ado2gh/agents/migration_agent/tools/shared_tools.py:35`) now writes `agent.tool.path_refused` with the prefix, the truncated endpoint and the reason through the `IdeAuditBridge` the agent's routes already use, which masks every value and swallows its own write failures (CA-004). No database is opened at import; the bridge creates its writer on first event. Regression: `tests/unit/test_tool_bindings.py::test_join_api_path_refusal_is_audited`.
...
- contract_change: false — internal; `tests/contract/public_surface_snapshot.json` unchanged.
- reopened_and_closed: 2026-09-13, same day, by Claude (opus subagent, R10b), commit `9719609`. The same second-round review of this function's territory found an adjacent defect: `fetch_github_workflow` (`ado2gh/agents/migration_agent/tools/validator_tools.py`) decoded the base64 file content and truncated it to `MAX_WORKFLOW_CONTENT_CHARS` before any redaction ran, so a secret straddling that boundary could be cut into a fragment too short for `redact_text`'s patterns to recognise — and the tool result is what gets persisted (THR-02-001). `redact_text` now runs on the full decoded content first, and the cap is applied to the masked string, so `truncated` also reflects the masked length rather than the raw one. Regression: `tests/unit/test_tool_bindings.py::test_fetch_github_workflow_redacts_before_it_truncates` (a 52-character PAT placed to straddle the cap). Revert proof at `run-r10b2-revert-tools.txt`, `16 failed, 37 passed`.
- closed_on: 2026-09-13
...
- reopened_and_closed: 2026-09-13, same day, by Claude (opus subagent, R10b), commit `bf1a07c`. The `fence_untrusted`/`redact_payload` fix above covered the planner and validator conversation batches and their checkpoint copies, but not a third sink reachable through the same research loop: `_discovery_snapshot` (`ado2gh/agents/migration_agent/nodes/planner_research.py:46`, GAP-096) writes its matched tool result straight into `session["discovery_snapshot"]`, which `nodes/intent.py:70-84` later renders into the system prompt — a durable, unmasked copy independent of the conversation-batch path this entry closed first. `_discovery_snapshot` now returns `redact_payload(result)` at both call sites (the live research loop and the dormant duplicate branch in `_execute_planner_tool_call`). Regression: `tests/unit/test_planner_node.py::test_discovery_snapshot_is_redacted_before_it_enters_session_state`. Revert proof at `run-r10b2-revert-discovery.txt`, `1 failed, 28 passed`.
...
- reopened_and_closed: 2026-09-13, same day, by Claude (opus subagent, R10b), commit `9719609`. The two pre-existing path-containment raise sites in `join_api_path` (`endpoint_too_long`, `endpoint_escapes_prefix`) previously raised `ApiPathError` with no record beyond the tool result this entry already redacts. The same commit that closed GAP-089's fragment gap routes all three raise sites — including the new fragment check — through one `_refuse_path` helper, so every path-containment refusal now carries a fixed, `redact_text`-safe reason and an audit record (`agent.tool.path_refused`, CA-004) instead of a per-site message this entry's masking was not written against. Regression: `tests/unit/test_tool_bindings.py::test_join_api_path_refusal_is_audited`. Revert proof at `run-r10b2-revert-tools.txt`, `16 failed, 37 passed`. See GAP-089 for the fragment-traversal fix itself.
...
- reopened_and_closed: 2026-09-13, same day, by Claude (opus subagent, R10b), commit `bf1a07c`. Not a gap in this entry's own route-matching gate — it still requires the exact `/v1/settings/profiles/{id}/discovery` shape — but the same function stores whatever that route returns verbatim into durable session state, and GAP-091's masking pass had not yet reached this sink. `_discovery_snapshot` now returns `redact_payload(result)` before the caller stores it, at both call sites. Regression: `tests/unit/test_planner_node.py::test_discovery_snapshot_is_redacted_before_it_enters_session_state`. Revert proof at `run-r10b2-revert-discovery.txt`, `1 failed, 28 passed`. See GAP-091 for the masking-choke-point fix itself.
...
### GAP-097 (GAP-ACC-13) A caller-supplied query string on the GitHub proxy routes is silently dropped rather than forwarded upstream

- components: accelerator service
- violates: Principle IV (a docstring and a caller's reasonable expectation both say the query string travels; measured behaviour says otherwise); Principle I (Clean Code — misleading code: two different `.split("?", 1)[0]`/`partition("?")` calls read as query-string handling but only ever fire on a percent-encoded `?` that a caller placed inside the path segment itself, not on an ordinary URL query string)
- evidence:
  - Measured against the installed FastAPI/Starlette stack in this session (`TestClient`): a normal request `GET /v1/github/repos/o/r/contents/x?ref=main` arrives at the handler with `path == "repos/o/r/contents/x"` and `request.query_params == {"ref": "main"}` — Starlette splits the query string out of the `{path:path}` capture before routing runs, so it never reaches the handler through `path` at all. `grep -n "query_params" services/accelerator_api/routes/proxy_routes.py` returns no matches: no route handler in this file — `proxy_ado_get`, `proxy_github_get/post/patch/put/delete` — ever reads `request.query_params`, though every one declares `request: Request`.
  - `_github_endpoint` (`services/accelerator_api/routes/proxy_routes.py:117-129`) strips anything from a literal `?` onto the end of the decoded path (`:129`), and the GET dispatch in `_proxy_github_request` (`:215`) calls `gh._get(endpoint)` with no params argument at all, so a GitHub proxy call's query string — real or percent-encoded-into-path — is never forwarded upstream.
  - The ADO proxy route (`:88`, `:102`, `:206`) is no better off for a genuine caller: its own `partition("?")` only ever fires on the same narrow percent-encoded-into-path construction, which `tests/integration/test_proxy_routes.py::test_ado_get_parses_a_query_string_encoded_into_the_path` exercises and pins. A plain `?a=b` on the request URL is dropped before the handler runs, which `::test_a_plain_query_string_does_not_reach_the_upstream_call` already pins as "observed behaviour" against the handler's own docstring, which promises "any query string on it is passed along"; that test's comment already recorded the divergence as "carried as a follow-up rather than fixed here" without a GAP id to carry it.
- severity: low. No route's authorization depends on a query parameter — `require_operate` and `require_approve_live_execution` gate on the HTTP method and session state, not on `request.query_params` — so this is a fidelity defect, not an auth bypass; the practical effect is a caller-supplied `ref`, `per_page` or similar parameter being silently dropped, so a read answers from the default branch or default paging instead of the one asked for, and a write or delete that depends on such a parameter would silently proceed without it rather than fail loudly.
- blast_radius: every GitHub proxy route (`proxy_github_get/post/patch/put/delete`) for any caller that supplies a real query string; the ADO proxy route for the same case.
- status: open
- resolution: —
- regression_check: —
- revert_proof: —
- contract_change: false — no route, CLI command, table or environment variable would change; forwarding an already-declared `Request`'s existing `query_params` is additive.
- follow_up: forward query parameters through the proxy allowlist — read `request.query_params` in each handler (or centrally in `_proxy_github_request`/the ADO handler) and merge them into the upstream call under the same allowlist discipline `require_operate`/`require_approve_live_execution` already apply to these routes, rather than relying on the percent-encoded-into-path construction these handlers happen to parse today.
- closed_on: —
