...
- [ ] T080 [P] [US4] G-seed 12 (if high): fix deployment/CI findings from T033 (e.g. secrets passed as build args → runtime env; `migrate-repo.yml` requires an approval gate/environment before any non-dry-run step); regression check = a `tests/unit/test_gap_NNN_ci_artifacts.py` asserting the workflow YAML shape with `yaml.safe_load`
