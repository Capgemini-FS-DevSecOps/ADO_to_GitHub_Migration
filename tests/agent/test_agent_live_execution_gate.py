...
def test_operator_live_execute_pev_blocked_without_approval(authed_operator_client):
    ...
    with patch("services.agent.routes.session_routes._accel_get", new_callable=AsyncMock) as mock_get:
        mock_get.return_value = {"repos": [], "repos_scanned": 0}
        with patch("services.agent.routes.session_routes._build_migration_plan", new_callable=AsyncMock) as mock_plan:
