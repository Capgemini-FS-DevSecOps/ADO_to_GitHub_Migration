...
def test_create_session_chat_only_does_not_call_accelerator():
    ...
    with patch("services.agent.main._accel_post", new_callable=AsyncMock) as mock_post, patch(
        "services.agent.main._accel_get", new_callable=AsyncMock,
    ) as mock_get:
        r = client.post(
            "/v1/sessions",
...
def test_run_pev_requires_plan_confirmation():
    ...
    with patch(
        "services.agent.main._build_migration_plan", new_callable=AsyncMock,
    ) as mock_build, patch(
        "services.agent.main._accel_get", new_callable=AsyncMock,
    ) as mock_get:
...
def test_message_orchestrator_discovers_and_plans():
    ...
    with patch(
        ...
        "services.agent.main._accel_post", new_callable=AsyncMock,
    ) as mock_post:
