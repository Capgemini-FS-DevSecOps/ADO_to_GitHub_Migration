...
def test_dry_run_pipeline_finishes_with_dry_run_complete_status():
    ...
    with patch.object(pr, "_prereq_checker") as mock_checker:
        mock_checker.check.return_value = (True, [])
        with patch.object(pr, "_step_connect"), patch.object(
            pr, "_step_analyze_deps",
        ), patch.object(pr, "_migrate_scoped"), patch.object(pr, "_step_validate"):
            pr._execute(run.id, None)
