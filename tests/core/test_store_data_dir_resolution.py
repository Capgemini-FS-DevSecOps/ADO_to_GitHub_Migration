...
``LLMModelStore`` and one ``ConnectivityStore`` at module import and keeps them for the
process lifetime. While those constructors resolved ``ADO2GH_DATA_DIR`` eagerly, every
later change to the variable was ignored by anything reached through a route: whichever
directory happened to be current at import time was read and written for every request
after it — leaking one deployment's profile, and one test's, into the next.

The connectivity store's own version of this lives in
``tests/core/test_connectivity_store.py``; these two cover its siblings.
"""
