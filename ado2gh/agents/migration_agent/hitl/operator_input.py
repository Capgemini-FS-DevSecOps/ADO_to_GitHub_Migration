...
def operator_input_from_blockers(
    ...
    *,
    ...
    resolution_options = resolution_options_from_context(blockers=blockers)
...
def validation_failures_from_baseline_probes(
    ...
    planner_style = [f for f in baseline_findings if isinstance(f, dict) and not f.get("dry_run")]
    for blocker in blockers_from_baseline_probes(planner_style):
        failures.append({
    ...
    options = [
        {
            "value": "replan",
            "label": "Replan",
            "description": "Revise the migration plan using validator evidence.",
            "recommended": True,
        },
        {
            "value": "skip_blocked_scope",
            "label": "Skip blocked scope",
            "description": "Continue without the failing scope.",
            "recommended": False,
        },
        {
            "value": "continue",
            "label": "Continue anyway",
            "description": "Proceed despite validation warnings (use only when risk is accepted).",
            "recommended": False,
        },
    ]
