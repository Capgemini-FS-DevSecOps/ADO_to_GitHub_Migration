...
def clear_stale_plan_approval(
    ...
    *,
    ...
    (THR-09-002). A plan already handed to the executor normally keeps its
    approval, so an in-flight or finished run is never retro-actively unapproved.

    ``replaced_plan`` is for the one path that is not that normal replan: a tool
    that stores a brand-new plan outright, outside the planner-approval flow
    that would otherwise have re-checked the approval already. That plan was
    never shown to the operator, so whatever run was in flight belongs to a plan
    that no longer exists (GAP-129) — the exemption for an in-flight run does not
...
def build_dynamic_form(
    *,
    ...
    """Build a sanitized dynamic form from field specs + context recommendations."""
    from ado2gh.agents.migration_agent.hitl.form_fields import field_dict_from_spec

    ctx = dict(planner_context or {})
    missing = [f.name for f in fields]
    if not ctx.get("field_recommendations"):
        from ado2gh.agents.migration_agent.hitl.form_fields import build_field_recommendations

        ctx["field_recommendations"] = build_field_recommendations(
            ctx.get("session") or {},
            repo_suggestions=ctx.get("repo_suggestions"),
            missing_fields=missing,
        )

    desc = description
    if ctx.get("repo_suggestions") and "repository_id" in missing:
        hint = ", ".join(ctx["repo_suggestions"][:5])
        if hint and hint not in desc:
            desc = f"{desc}\n\nSuggested repositories: {hint}".strip()

    form_fields = [field_dict_from_spec(spec, planner_context=ctx) for spec in fields]
    ...
    *,
    ...
    if session.get("migration_plan"):
        ...
        if intake.confirm_execute:
            from ado2gh.agents.migration_agent.hitl.blockers import needs_blocker_resolution, outstanding_blockers
            from ado2gh.agents.migration_agent.hitl.operator_input import (
    ...
    if intake_ready_for_planner(intake):
        ...
        return {
            ...
            "dry_run": dry_run,
            "message": f"Proceeding to build the migration plan for '{repo_id}' in {mode} mode.",
        }
