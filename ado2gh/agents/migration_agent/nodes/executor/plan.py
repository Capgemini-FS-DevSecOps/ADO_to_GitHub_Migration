...
def resolve_github_org(
    *,
    ...
    """Resolve target GitHub org from repo, session, plan, session profile, or migration.yaml."""
    candidates: list[Any] = []
    if repo:
    ...
    try:
        ...
        store = SettingsStore()
        settings = store.load()
        config_path = settings.advanced.config_path or "migration.yaml"
        global_cfg, _ = ConfigLoader.load(config_path)

        profile_id = str((session or {}).get("profile_id") or "").strip()
        session_profile = store.get_profile(profile_id) if profile_id else None
        discovery = (session or {}).get("discovery_snapshot")
        scan = discovery if isinstance(discovery, dict) else None

        for profile in [session_profile, store.get_active_profile()]:
            org = resolve_gh_org(
                profile,
                global_cfg=global_cfg,
                config_path=config_path,
                scan=scan,
                profile_id=profile_id or (profile.id if profile else None),
            )
            if org:
                return org
