...
- **Why a worktree**: the shared working tree carries an unrelated, uncommitted reversion of `ado2gh/agents/migration_agent/nodes/executor/node.py` (the GAP-006 fix), written at 12:52 by something outside this increment's session; with it in place `tests/feature` stalls at the 17th test. That file is not part of this commit.
